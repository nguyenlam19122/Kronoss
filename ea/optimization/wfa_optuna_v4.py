"""
Giong wfa_optuna_v3 NHUNG entry = ATR-cross cua EA goc (khoa fast5/0.5, slow10/3) + loc may.
Toi uu CUNG 6 tham so quan ly von: sl_atr_period, sl_mult, tp_mult, trail_period, trail_mult,
use_ichi_exit. Muc tieu/loc/13-fold/Optuna NSGA-II giong het v3 -> so sanh cong bang voi v3.
So sanh DEFAULT (TP cung) vs GONG (bo TP, Slow Trail).
"""
import argparse, json, os, numpy as np
import optuna
import backtester_v4 as b4
...
ENTRY_FIXED = {"fast_period": 5, "fast_mult": 0.5, "slow_period": 10, "slow_mult": 3.0}
...
def suggest(trial, variant):
    ...
    p.update(ENTRY_FIXED); p["risk"] = RISK
    if variant == "default":
        p["use_tp"] = 1; p["tp_mult"] = trial.suggest_float("tp_mult", 1.5, 4.0, step=0.25)
    else:
...
def full_params(variant, params):
    p = dict(params); p.update(ENTRY_FIXED); p["risk"] = RISK
    if variant == "default":
        p["use_tp"] = 1
    else:
        p["use_tp"] = 0; p["tp_mult"] = 0.0
...
def run_fold(arr, cache, variant, t0, t1, py, n_trials, pop, seed):
    def objective(trial):
        p = suggest(trial, variant)
        pn, dd, ddp = b4.backtest_v4(arr, cache, p, t0, t1, START_BALANCE)
        return objective_value(b3.metrics_v3(pn, dd, ddp, py))
...
def evaluate(arr, cache, variant, params, s, e, dt):
    pn, dd, ddp = b4.backtest_v4(arr, cache, full_params(variant, params), s, e, START_BALANCE)
    return b3.metrics_v3(pn, dd, ddp, period_years(dt, s, e)), pn
...
def main():
    ...
    ap.add_argument("--outdir", default=os.path.join(os.path.dirname(__file__), "results_v4"))
    args = ap.parse_args()
    os.makedirs(args.outdir, exist_ok=True)

    print("Nap du lieu...")
    df = b4.load_data(args.csv); arr = b4.to_arrays(df)
    cache = b4.IndicatorCache(arr); yb = b4.year_bounds(arr)
    dt = df["dt"].dt.to_pydatetime().tolist()
    print(f"  {len(df):,} nen | {df['dt'].iloc[0]} -> {df['dt'].iloc[-1]}")
    _ = b4.backtest_v4(arr, cache, full_params("default",
        {"sl_atr_period": 14, "sl_mult": 2.0, "tp_mult": 2.0, "trail_period": 10,
         "trail_mult": 2.0, "use_ichi_exit": True}), yb[2010][0], yb[2011][1], START_BALANCE)

    out = {"default": [], "gong": []}; oos = {"default": [], "gong": []}
    for variant in ("default", "gong"):
        print(f"\n===== ENTRY ATR-CROSS | BIEN THE: {variant.upper()} =====")
        for k, ((tr0, tr1), te) in enumerate(FOLDS, 1):
            ...
            best, bv = run_fold(arr, cache, variant, t0, t1, period_years(dt, t0, t1),
                                args.trials, args.pop, args.seed)
            ...
            print(f"  Fold {k:2d} Te{te_lbl} | IS Sh={m_tr['sharpe']:.2f} PF={m_tr['profit_factor']:.2f} "
...
def _stitch(pl):
    ...
    if n == 0: return dict(trades=0, net=0.0, pf=0.0, dd=0.0, sharpe=0.0, wr=0.0, pnls=p)
...
def save(out, oos, outdir):
    json.dump(out, open(os.path.join(outdir, "wfa_v4_results.json"), "w"), indent=2, ensure_ascii=False)
    ...
    L = ["# WFA + Optuna (NSGA-II) — ENTRY ATR-cross (EA goc) | 1R=$20 gom spread\n",
         ...
         f"**Von ${START_BALANCE:.0f}, 1R=${RISK:.0f}.** Entry ATR-cross khoa fast5/0.5, slow10/3 + loc may.  ",
         "Toi uu 6 tham so quan ly von (giong v3): sl_atr_period, sl_mult, tp_mult, trail_period, "
         "trail_mult, use_ichi_exit.\n"]
    for variant, title in (("default", "DEFAULT (TP cung = tp_mult × R)"),
        ...
        L.append(f"\n**OOS ghep ({variant}):** net ${s['net']:.0f} ({s['net']/START_BALANCE*100:.0f}%), "
                 f"PF {fmt(s['pf'])}, MaxDD {s['dd']:.1f}%, Sharpe {s['sharpe']:.2f}, WR {s['wr']:.0f}%, "
                 f"{s['trades']} lenh. IS hop le {feas}/{len(out[variant])}, OOS co lai {win}/{len(out[variant])}.")

    d, g = st["default"], st["gong"]
    L.append("\n## So sanh DEFAULT vs GONG (OOS ghep)\n")
    L.append("| Chi so | DEFAULT | GONG |")
    L.append("|---|---|---|")
    L.append(f"| Net | ${d['net']:.0f} | ${g['net']:.0f} |")
    L.append(f"| Profit Factor | {fmt(d['pf'])} | {fmt(g['pf'])} |")
    L.append(f"| Max Drawdown % | {d['dd']:.1f}% | {g['dd']:.1f}% |")
    L.append(f"| Sharpe | {d['sharpe']:.2f} | {g['sharpe']:.2f} |")
    ...
    open(os.path.join(outdir, "wfa_v4_summary.csv"), "w").write("\n".join(rows))

    try:
        plot(st, outdir)
    except Exception as e:
        print("Loi ve bieu do:", e)
    print("\n----- OOS GHEP (entry ATR-cross) -----")
    for v in ("default", "gong"):
        s = st[v]
...
def plot(st, outdir):
    ...
    for v, col in (("default", "#E65100"), ("gong", "#1565C0")):
        p = st[v]["pnls"]; eq = np.concatenate(([START_BALANCE], START_BALANCE + np.cumsum(p)))
    ...
    ax.set_title("WFA OOS 2013-2026 (ENTRY ATR-cross EA goc) — DEFAULT vs GONG")
    ax.set_xlabel("So lenh OOS"); ax.set_ylabel("Balance ($)"); ax.legend(); ax.grid(alpha=0.3)
    fig.tight_layout(); fig.savefig(os.path.join(outdir, "oos_default_vs_gong_v4.png"), dpi=120)
    plt.close(fig)
